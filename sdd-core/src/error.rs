...
pub enum AppError {
    ...
    #[diagnostic(
        code(sdd::missing_field),
        help("Add the missing field to the YAML entry")
    )]
    MissingField {
        path: String,
        index: usize,
    ...
    #[diagnostic(
        code(sdd::missing_root_key),
        help("Add a top-level '{key}' key containing a list")
    )]
    MissingRootKey { path: String, key: String },

    #[error("Empty '{key}' list in '{path}'")]
    #[diagnostic(
        code(sdd::empty_list),
        help("Add at least one entry to the '{key}' list")
    )]
    EmptyList { path: String, key: String },

    #[error("Regex compilation error: {message}")]
    #[diagnostic(
        code(sdd::regex),
        help("This is an internal error — verify the regex pattern")
    )]
